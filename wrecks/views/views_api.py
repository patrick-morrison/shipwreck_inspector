...
def sites_csv(request):
    ...
    writer.writerow(['id', 'slug', 'name', 'sunk', 'built', 'built_details', 'description', 'construction', 'owner', 'size', 'location', 'underwater', 'sinking', 'region', 'latitude', 'longitude', 'image', 'image_caption', 'sketchfab_link', 'museum_link', 'dave_link', 'user_id', 'order'])

    sites = Site.objects.all()

    for s in sites:
        writer.writerow([s.id, s.slug, s.name, s.sunk, s.built, s.built_details, s.description, s.construction, s.owner, s.size, s.location, s.underwater, s.sinking, s.get_region_display(), s.latitude, s.longitude, s.image, s.image_caption, s.sketchfab_link, s.museum_link, s.dave_link, s.user.id, s.order])
